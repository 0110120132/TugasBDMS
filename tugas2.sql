...
3. Tampilkan produk-produk televisi
   SELECT * FROM produk WHERE nama_produk='TV';
...
5. Tampilkan pelanggan yang lahirnya bukan di Jakarta dan mengandung huruf ‘yo‘
   SELECT * FROM pelanggan WHERE tmp_lahir NOT LIKE = 'Jakarta' OR nama_pelanggan LIKE = '%yo%';
...
2. Tampilkan produk yang paling murah
   SELECT * FROM produk ORDER BY harga_jual;
...
5. Tampilkan pelanggan yang paling muda
   SELECT * FROM produk ORDER BY tgl_lahir DESC LIMIT 1;
6. Tampilkan pelanggan yang paling tua
   SELECT * FROM produk ORDER BY tgl_lahir LIMIT 1;
